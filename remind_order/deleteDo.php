...
try {
    ...
    $ptmt = $db->prepare("SELECT COUNT(*) as rt FROM `ordering` WHERE passwd = PASSWORD(:del_passwd) AND uid = :id");
    ...
    $dtmt = $db->prepare("DELETE FROM `ordering` WHERE uid = :id");
    $dtmt->bindParam(":id", $id, PDO::PARAM_INT);
    $result = $dtmt->execute();

    if ($result) {
        _goto("/remind_order/ordering.php?page=".$page);
    } else {
