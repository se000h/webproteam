...
if (empty($item)) { warn_back("인쇄품목을 선택해 주세요."); }
if (empty($count)) { warn_back("인쇄 매수를 작성해 주세요."); }
...
try {
    ...
    $stmt = $db->prepare("INSERT INTO `ordering`(subject, contents, passwd, secure, name, regdate, phonenum, email, emailadr, item, size, count) VALUES(:subject, :contents, :isPasswd, :isSecure, :name, :regdate, :phonenum, :email, :emailadr, :item, :size, :count)");
